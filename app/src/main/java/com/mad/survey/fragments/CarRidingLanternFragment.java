...
public class CarRidingLanternFragment extends BaseFragment implements View.OnClickListener , OnFragmentResumedListener{

    private TextView txtSubTitle;
    private TextView txtCarNumber;
    private CheckBox chkYes,chkNo;

    public static CarRidingLanternFragment newInstance(){
        CarRidingLanternFragment fragment = new CarRidingLanternFragment();
        return fragment;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View parent = inflater.inflate(R.layout.fragment_car_riding_lantern, container, false);
    ...
    private void goToNext(int yes){
        ...
        MADSurveyApp.getInstance().getCarData().setIsThereCDI(yes);
        carDataHandler.update(MADSurveyApp.getInstance().getCarData());
        if(yes == 1)
            ((BaseActivity) getActivity()).replaceFragment(BaseActivity.FRAGMENT_ID_CAR_RIDING_LANTERN_MOUNTING, "car_riding_lantern_mounting");
        else if(yes == 2)
            ((BaseActivity) getActivity()).replaceFragment(BaseActivity.FRAGMENT_ID_CAR_SEPERATEPI, "car_seperatepi");
