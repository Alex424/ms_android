...
public class CarHandrailFragment extends BaseFragment implements View.OnClickListener , OnFragmentResumedListener{

    private TextView txtSubTitle;
    private TextView txtCarNumber;
    private CheckBox chkYes,chkNo;

    public static CarHandrailFragment newInstance(){
        CarHandrailFragment fragment = new CarHandrailFragment();
        return fragment;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View parent = inflater.inflate(R.layout.fragment_car_handrail, container, false);
    ...
    private void updateUIs(){
        ...
        int yes = carData.getIsThereHandRail();
        if(yes == 1)
    ...
    private void goToNext(int yes){
        if(yes == 2){
            MADSurveyApp.getInstance().getCarData().setHandRailDistanceFromReturn(0.0);
            MADSurveyApp.getInstance().getCarData().setHandRailDistanceFromWall(0.0);
            MADSurveyApp.getInstance().getCarData().setHandRailHeightFromFloor(0.0);
        }
        MADSurveyApp.getInstance().getCarData().setIsThereHandRail(yes);
        carDataHandler.update(MADSurveyApp.getInstance().getCarData());
        if(yes == 1)
            ((BaseActivity) getActivity()).replaceFragment(BaseActivity.FRAGMENT_ID_CAR_MEASUREMENTS, "car_measurements");
        else if(yes == 2)
